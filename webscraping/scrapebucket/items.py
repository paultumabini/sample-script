...
class ScrapebucketItem(scrapy.Item):
    ...
    stock_number = scrapy.Field(input_processor=MapCompose(remove_char_from_str, str.strip), output_processor=TakeFirst())
    vin = scrapy.Field(input_processor=MapCompose(extract_vin, str.upper, str.strip), output_processor=TakeFirst())
    vehicle_url = scrapy.Field(output_processor=TakeFirst())
